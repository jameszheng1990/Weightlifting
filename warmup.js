(function(){
  ...
  function inject(){
    ...
    if(!root) return;
    root.querySelectorAll(".warmup-box").forEach(e=>e.remove());
  ...
  const obs=new MutationObserver(()=>{clearTimeout(window.__warmupTimer);window.__warmupTimer=setTimeout(inject,20)});
  window.addEventListener("DOMContentLoaded",()=>{const root=document.getElementById("content");if(root)obs.observe(root,{childList:true,subtree:true});setTimeout(inject,80)});
