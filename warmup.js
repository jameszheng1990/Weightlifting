(function(){
  ...
  function box(lines,compact){
    ...
    d.innerHTML=`<div class="warmup-title">热身${compact?"（已热身，缩短）":""} <span>逐组记录</span></div><div class="warmup-steps">${lines.map(x=>`<span>${x}</span>`).join("<b>→</b>")}</div>`;
    return d;
  }
  ...
  obs=new MutationObserver(muts=>{
    const relevant=muts.some(m=>[...m.addedNodes,...m.removedNodes].some(n=>n.nodeType===1&&!n.matches?.('.warmup-box,.warmup-record,.set-tracker,.feature-actions,.load-btn')&&!n.closest?.('.warmup-box,.warmup-record,.set-tracker')));
    if(!relevant)return;
