...
function updateStickyTop(){const tabs=$('#loaderTabBar'),result=$('.reverse-result');if(!tabs||!result)return;const cssTop=parseFloat(getComputedStyle(tabs).top)||0;const top=Math.max(0,cssTop+tabs.offsetHeight+8);document.documentElement.style.setProperty('--reverse-sticky-top',`${top}px`)}
function installSticky(){const result=$('.reverse-result');if(!result)return;result.classList.add('reverse-result-sticky');updateStickyTop();window.addEventListener('resize',updateStickyTop,{passive:true});window.addEventListener('orientationchange',()=>setTimeout(updateStickyTop,120),{passive:true})}
...
function show(which){const target=which==='target';$('#loaderTargetSubView')?.classList.toggle('loader-hidden',!target);$('#loaderReverseSubView')?.classList.toggle('loader-hidden',target);$('#loaderSubTarget')?.classList.toggle('active',target);$('#loaderSubReverse')?.classList.toggle('active',!target);if(!target){paintTotal();setTimeout(updateStickyTop,0)}}
...
<div class="reverse-result"><span>当前总重量</span><b id="reverseTotal">—</b><div class="small" id="reverseBreakdown"></div></div>
...
render('kg');render('lb');syncReverseSetup();installSticky();return true}
function boot(){if(init())return;let n=0;const id=setInterval(()=>{n++;if(init()||n>30)clearInterval(id)},100)}
window.addEventListener('DOMContentLoaded',boot);window.addEventListener('pageshow',()=>setTimeout(()=>{init();updateStickyTop()},120));
})();