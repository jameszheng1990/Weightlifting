...
function touch(row){let s=loadActive(),now=Date.now(),meta=rowMeta(row);if(s&&(s.date!==ymd()||s.week!==meta.week||s.day!==meta.day)){archive(s,s.lastActive||now);s=null}if(!s)s=newSession(row);if(s.pausedAt){s.pausedMs=(s.pausedMs||0)+(now-s.pausedAt);s.pausedAt=null}s.lastActive=now;saveActive(s);ensureStopwatch();renderAll()}
function pauseResume(){const s=loadActive();if(!s)return;const now=Date.now();if(s.pausedAt){s.pausedMs=(s.pausedMs||0)+(now-s.pausedAt);s.pausedAt=null}else s.pausedAt=now;s.lastActive=now;saveActive(s);renderAll()}
function finish(){const s=loadActive();if(!s)return;if(!confirm('结束本次训练并保存总用时？'))return;const now=Date.now();if(s.pausedAt){s.pausedMs=(s.pausedMs||0)+(now-s.pausedAt);s.pausedAt=null}archive(s,now);saveActive(null);renderAll()}
...
function injectDaySummaries(){const active=loadActive();$$('#content>.card').forEach(card=>{const meta=cardMeta(card);if(!meta.day)return;let box=card.querySelector('.day-session-summary');if(!box){box=document.createElement('div');box.className='day-session-summary';const h=card.querySelector('h3');h?.insertAdjacentElement('afterend',box)}const saved=dayTotal(meta),isActive=active&&Number(active.week)===meta.week&&active.day===meta.day;const live=isActive?Math.round(elapsedMs(active)/1000):0,total=saved+live;box.innerHTML=`<div><span>总用时</span><b>${total?duration(total):'—'}</b>${saved&&!isActive?'<small>已保存</small>':isActive?'<small>进行中</small>':''}</div>${isActive?'<button type="button" class="day-session-end">结束训练</button>':''}`;box.querySelector('.day-session-end')?.addEventListener('click',finish)})}
function removeReviewTab(){const t=$('#tabToday');if(t)t.remove();$('#reviewView')?.remove()}
function hideOldCountdown(){const r=$('#restTimer');if(r)r.classList.add('session-force-hide');const heavy=$('#timerHeavy');const card=heavy?.closest('.tools-card');if(card)card.style.display='none';const intro=$('#toolsView .tools-card .small');if(intro&&/计时器|Quick Load|训练复盘/.test(intro.textContent))intro.textContent='组数记录、e1RM 和备份。'}
function renderAll(){renderStopwatch();injectDaySummaries()}
function init(){ensureStopwatch();removeReviewTab();hideOldCountdown();renderAll();clearInterval(tickId);tickId=setInterval(renderAll,1000);const root=$('#content');if(root)new MutationObserver(()=>{if(injectBusy)return;injectBusy=true;setTimeout(()=>{injectBusy=false;injectDaySummaries()},60)}).observe(root,{childList:true,subtree:true})}
