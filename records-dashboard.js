...
function chartCard(title,pts,unit){
  const avg=pts.length?pts.reduce((sum,p)=>sum+p.value,0)/pts.length:null,hasMany=pts.length>20;
  return `<div class="records-chart-card"><div class="records-chart-head"><div class="records-chart-title">${title}</div><div class="records-chart-head-right"><div class="records-chart-latest">${avg!==null?`平均 ${fmt(avg,unit==='kcal'?0:1)} ${unit}`:'暂无数据'}</div>${pts.length?`<button type="button" class="records-span-toggle" data-mode="detail">整体趋势</button>`:''}</div></div><div class="records-chart-detail${hasMany?' is-scrollable':''}"><div class="records-chart-scroll">${svgChart(pts,unit,false)}</div>${hasMany?'<div class="records-scroll-hint">左右滑动查看更多数据点</div>':''}</div><div class="records-chart-overview" hidden>${svgChart(pts,unit,true)}</div></div>`
}
function hideLegacyE1RM(){const best=$('#e1rmBest');const card=best?.closest('.tools-card');if(card)card.style.display='none'}
function render(){const tools=$('#toolsView');if(!tools)return false;hideLegacyE1RM();let box=$('#recordsDashboard',tools);if(!box){box=document.createElement('div');box.id='recordsDashboard';box.className='tools-card records-dashboard';const first=tools.querySelector('.tools-card');if(first)first.insertAdjacentElement('afterend',box);else tools.prepend(box)}const wp=metricPoints('weight'),cp=metricPoints('calories'),sp=metricPoints('sleep'),mp=cardioMilesPoints();box.innerHTML=`${chartCard('Body Weight',wp,'kg')}${chartCard('Calories Burned',cp,'kcal')}${chartCard('Daily Miles',mp,'mi')}${chartCard('Sleep',sp,'h')}<div class="records-note">超过 20 个数据点时可横向滑动逐点查看；“整体趋势”会把全部数据压缩到一屏。Daily Miles 会自动汇总同一天所有有氧记录。</div>`;return true}
